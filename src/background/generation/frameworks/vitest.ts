...
export class VitestFrameworkAdapter implements TestFrameworkAdapter {
  ...
  render(tests: GeneratedTestCase[], projectMeta: ProjectMeta): GeneratedFile[] {
    ...
    for (const test of tests) {
      ...
      if (!grouped.has(key)) {
        grouped.set(key, []);
        groupToPath.set(key, `tests/${pathMeta.resource}/${test.request.method.toLowerCase()}-${pathMeta.leaf}.test.ts`);
      }
...
${idempotencyCheck}
      ...
      return {
        path: groupToPath.get(groupKey) ?? `tests/${groupKey}.test.ts`,
        content: `/**
...
`
  ...
  renderReadme(projectMeta: ProjectMeta): GeneratedFile {
    ...
    return {
      path: 'README.md',
      content: `# APItiser Generated Vitest Tests
...
API_BASE_URL=http://localhost:3000 npx vitest run
\`\`\`
${readinessNotes}`
    };
  }
